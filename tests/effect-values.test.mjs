...
test('normalizeEffectChange: ADD onto a CSB prop keeps ADD with a numeric delta', () => {
...
test('normalizeEffectChange: ADD accepts bare prop math without ${ }$ wrappers', () => {
  ...
  assert.equal(result.mode, 2);
  assert.equal(result.value, '${ ATK_calc * 0.08 }$');
});
...
test('normalizeEffectChange: MULTIPLY onto a CSB prop keeps MULTIPLY with a numeric factor', () => {
...
test('normalizeEffectChange: restores legacy current+() to native ADD', () => {
...
test('normalizeEffectChange: restores legacy current*() to native MULTIPLY', () => {
  const result = normalizeEffectChange({
...
test('normalizeEffectChange: re-normalizing ADD/MULTIPLY deltas stays stable', () => {
...
test('normalizeEffectChange: prop-key CUSTOM math migrates to native ADD/MULTIPLY', () => {
  const add = normalizeEffectChange({
    key: 'system.props.DEF',
    mode: 0,
    value: '${ DEF + (ATK_calc * 0.08) }$'
  });
  assert.equal(add.mode, 2);
  assert.equal(add.value, '${ ATK_calc * 0.08 }$');
  ...
  assert.equal(mul.mode, 1);
  assert.equal(mul.value, '2');
});
